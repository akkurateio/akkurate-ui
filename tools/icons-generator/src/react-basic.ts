...
export const ${name}:FunctionComponent<IProps> = ({
    size = IconSize.md,
    className = ''
}) => {
  return (
    <svg width={size} height={size} viewBox={'0 0 32 32'} fill={'currentColor'} className={className} xmlns={'http://www.w3.org/2000/svg'}>
        ${svg}
    </svg>
  )
}`
